...
var_bounds = np.ones((T, 2)) * var_bounds
    ...
    def test_cgarch(self):
        ...
        q2 = np.ndarray(T)
        g2 = np.ndarray(T)
        recpy.cgarch_recursion(parameters, fresids, sigma2, backcast, self.var_bounds, g2, q2)
        sigma2_numba = sigma2.copy()
        recpy.cgarch_recursion_python(parameters, fresids, sigma2, backcast, self.var_bounds, g2, q2)
        sigma2_python = sigma2.copy()
        rec.cgarch_recursion(parameters, fresids, sigma2, backcast, self.var_bounds, g2, q2)
