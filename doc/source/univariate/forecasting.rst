...
All models suppoer three methods of forecasting:
  ...
  * Simulation: simulation-based forecasts are always available for any
    ...
    to forward simulate using the assumed distribution of residusals, e.g., a
    Normal or Student's t.
  * Bootstrap: bootstrap-based forecasts are similar to simulation based
    forecasts except that they makse use of the standardized residuals
...
Assume there are :math:`B` simulationed paths.  A single simulated path is
generated using
...
first residus, :math:`\epsilon_{t}`, is in-sample and so is not simulated.
...
except that the standardized residuals are generated by the model.  Theese
standardized residuals are generated using the observed data and the
estiamted parameters as
...
is attached to a model fit result.`
  ...
  * ``simulations`` - A non-negative ineger indicating the number of
