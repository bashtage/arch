...
class OaxacaBlinder(object):
    ...
    def __init__(self, endog, exog, bifurcate, hasconst=True,
        ...
        if self.var is True:
            ...
            self.f_cov = np.cov(exog_f.T)
            self.s_cov = np.cov(exog_s.T)
    ...
    def variance(self, decomp_type):
        ...
        if decomp_type == 3:
            char_var = (((self.exog_f_mean - self.exog_s_mean) @ self._f_model.cov_params() 
                        @ (self.exog_f_mean - self.exog_s_mean))
                        + (self._s_model.params @ (self.f_cov + self.s_cov)
                        @ self._s_model.params))

            coef_var = ((self.exog_s_mean 
                        @ (self._f_model.cov_params() + self._s_model.cov_params()) 
                        @ self.exog_s_mean) 
                        + ((self._f_model.params - self._s_model.params) @ self.s_cov 
        ...
        if decomp_type == 2:
            ...
            explained_var = (((self.exog_f_mean - self.exog_s_mean) @ ((self.weight[0] @ self._f_model.cov_params() @ self.weight[0])
                            + (self.weight[1] @ self._s_model.cov_params() @ self.weight[1])) @ (self.exog_f_mean - self.exog_s_mean))
                            + ((self.weight[0] @ self._f_model.params + self.weight[1] @ self._s_model.params)
                            @ (self.f_cov + self.s_cov)
                            @ (self.weight[0] @ self._f_model.params + self.weight[1] @ self._s_model.params)))
            
            unexplained_var = ((self.weight[1] @ self.exog_f_mean + self.weight[0] @ self.exog_s_mean)
                            @ (self._f_model.cov_params() + self._s_model.cov_params())
                            @ (self.weight[1] @ self.exog_f_mean + self.weight[0] @ self.exog_s_mean)
                            + ((self._f_model.params - self._s_model.params)
                            @ (self.weight[1] @ self.f_cov @ self.weight[1] + self.weight[0] @ self.s_cov @ self.weight[0])
                            @ (self._f_model.params - self._s_model.params)))
    ...
    def two_fold(self, two_fold_type = 'pooled', submitted_weight = None):
        ...
        two_fold_type: string, optional
            ...
            (a normal regression) is the non-discriminatory model. This includes
            the indicator variable. This is generally the best idea. If you have
            economic justification for using others, then use others.

            nuemark - This is similar to the pooled type, but the regression
            is not done including the indicator variable.
            
            cotton - This type uses the adjusted in Cotton (1988), which accounts
            for the undervaluation of one group causing the overevalution
            of another. It uses the sample size weights for a linear combination
            of the two model parameters
            
            reimers - This type uses a linear combination of the two models with both
            parameters being 50% of the non-discriminatory model.
            
            self_submitted - This allows the user to submit their own weights. Please be
            sure to put the weight of the larger mean group first. This should be
            submitted in the submitted_weights variable.

        submitted_weight: int/float, required if using self_submitted method, else optional
            This is the submitted weight for the larger mean. If the weight for the
            larger mean is p, then the weight for the other mean is 1-p. Only submit
            the first value.
        ...
        if two_fold_type == 'cotton':
            ...
            self.t_params = ((self.len_f / (self.len_f + self.len_s) 
            ...
            if submitted_weight == None:
                raise ValueError('Please submit weights if using self_submitted')
            submitted_weight = [submitted_weight, 1 - submitted_weight]
            self.t_params = (submitted_weight[0] * self._f_model.params 
            ...
            submitted_weight = self.t_params
        ...
        return OaxacaResults((self.unexplained, self.explained, self.gap), 2, var_val = var_val)
